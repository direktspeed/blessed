 ...
 * Program
 */
export default class Program extends EventEmitter {
    constructor(options) {
        super();
        const self = this;

        this.options = (!options || options.__proto__ !== Object.prototype) ? ({
            input: arguments[0],
            output: arguments[1]
        }) : options;
        ...
        options.log = this.options.log || this.options.dump;
        if (options.log) {
...
Program.prototype.log = function(...args) {
    return this._log('LOG', util.format(...args));
};
Program.prototype.debug = function(...args) {
...
Program.prototype.setupTput = function () {
    ...
    Object.keys(tput).forEach(key => {
        ...
        if (tput.padding) {
            ...
            self.put[key] = (...args) => {
                return self._write(tput[key](...args));
            };
        }
    });
};
...
Program.prototype._listenInput = function () {
    ...
    // Input
    this.input.on('keypress', this.input._keypressHandler = (ch, key = { ch }) => {
...
Program.prototype.getTextParams = function (param, callback) {
    return this.response('text-params', `\x1b]${param};?\x07`, (err, {pt}) => {
...
// Save Cursor Locally
Program.prototype.lsaveCursor = function(key = 'local') {
...
Program.prototype.charset = function(val, level = 0) {
    // See also:
    // acs_chars / acsc / ac
...
Program.prototype.sm =
    Program.prototype.setMode = function(...args) {
        const param = slice.call(args).join(';');
...
Program.prototype.rm =
    Program.prototype.resetMode = function(...args) {
        const param = slice.call(args).join(';');
        return this._write(`\x1b[${param || ''}l`);
    };
...
//   Tracking.
Program.prototype.initMouseTracking = function(...args) {
    return this._write(`\x1b[${slice.call(args).join(';')}T`);
};
// CSI > Ps; Ps T
...
Program.prototype.resetTitleModes = function(...args) {
    return this._write(`\x1b[>${slice.call(args).join(';')}T`);
};
...
Program.prototype.mc =
    Program.prototype.mediaCopy = function(...args) {
        return this._write(`\x1b[${slice.call(args).join(';')}i`);
    };
...
//   initial values.
Program.prototype.setResources = function(...args) {
    return this._write(`\x1b[>${slice.call(args).join(';')}m`);
};
// CSI > Ps n
...
Program.prototype.decscl =
    Program.prototype.setConformanceLevel = function(...args) {
        return this._write(`\x1b[${slice.call(args).join(';')}"p`);
    };
...
Program.prototype.restorePrivateValues = function(...args) {
    return this._write(`\x1b[?${slice.call(args).join(';')}r`);
};
...
Program.prototype.deccara =
    Program.prototype.setAttrInRectangle = function(...args) {
        return this._write(`\x1b[${slice.call(args).join(';')}$r`);
    };
// CSI ? Pm s
//   Save DEC Private Mode Values.  Ps values are the same as for
//   DECSET.
Program.prototype.savePrivateValues = function(...args) {
    return this._write(`\x1b[?${slice.call(args).join(';')}s`);
};
// CSI Ps ; Ps ; Ps t
//   Window manipulation (from dtterm, as well as extensions).
//   These controls may be disabled using the allowWindowOps
...
Program.prototype.decrara =
    Program.prototype.reverseAttrInRectangle = function(...args) {
        return this._write(`\x1b[${slice.call(args).join(';')}$t`);
    };
// CSI > Ps; Ps t
...
Program.prototype.setTitleModeFeature = function(...args) {
    return this._twrite(`\x1b[>${slice.call(args).join(';')}t`);
};
// CSI Ps SP t
...
Program.prototype.deccra =
    Program.prototype.copyRectangle = function(...args) {
        return this._write(`\x1b[${slice.call(args).join(';')}$v`);
    };
// CSI Pt ; Pl ; Pb ; Pr ' w
...
Program.prototype.decefr =
    Program.prototype.enableFilterRectangle = function(...args) {
        return this._write(`\x1b[${slice.call(args).join(';')}'w`);
    };
...
Program.prototype.decfra =
    Program.prototype.fillRectangle = function(...args) {
        return this._write(`\x1b[${slice.call(args).join(';')}$x`);
    };
// CSI Ps ; Pu ' z
...
Program.prototype.decelr =
    Program.prototype.enableLocatorReporting = function(...args) {
        return this._write(`\x1b[${slice.call(args).join(';')}'z`);
    };
// CSI Pt; Pl; Pb; Pr$ z
...
Program.prototype.decera =
    Program.prototype.eraseRectangle = function(...args) {
        return this._write(`\x1b[${slice.call(args).join(';')}$z`);
    };
// CSI Pm ' {
...
Program.prototype.decsle =
    Program.prototype.setLocatorEvents = function(...args) {
        return this._write(`\x1b[${slice.call(args).join(';')}'{`);
    };

/**
 * NONE EASY TRANSLATE ABLE PART
 * NONE EASY TRANSLATE ABLE PART
 * NONE EASY TRANSLATE ABLE PART
 * NONE EASY TRANSLATE ABLE PART
 * 
...
export function unshiftEvent(obj, event, listener) {
